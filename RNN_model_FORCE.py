...
class RNN:
	...
	def FORCE_train_next(self, function, num_iters, alpha = 1.0,
	...
	def FORCE_train(self, time, function, alpha = 1.0, timesteps_per_update = 3, train_output = 0, 
		...
		for t in tqdm(range(time), position = 0, leave = True):
			curr_time = self.timestep
			if curr_time % timesteps_per_update == 0 and curr_time != 0:
				self.FORCE_train_next(function, num_iters, alpha, train_output, train_weights)
				num_iters += 1
			else:
				self.next()
